...
class PostsViewTests(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        for i in range(1, 13):
            Post.objects.create(text=f"{i} Тестовый пост", author=cls.user, group=cls.group)
        for i in range(14, 20):
            Post.objects.create(text=f"{i} Тестовый пост", author=cls.author, group=cls.group2)
    ...
    def test_posts_view_index_transmits_correct_context(self) -> None:
        ...
        self.assertEqual(response.context.get(key="page_obj").object_list, post_list[:10])
        self.assertEqual(response.context.get(key="page_obj").object_list[0].image, self.post.image)

        self.assertEqual(len(response.context.get(key="page_obj")), len(post_list[:10]))
        response = self.guest_client.get(path=url + "?page=2")
        self.assertEqual(len(response.context.get(key="page_obj")), len(post_list[10:20]))
    ...
    def test_posts_view_group_posts_transmits_correct_context(self) -> None:
        ...
        self.assertEqual(response.context.get(key="title"), f"{self.group.title} | Yatube")
    ...
    def test_posts_view_post_detail_transmits_correct_context(self) -> None:
        ...
        self.assertEqual(response.context.get(key="title"), f"Пост {self.post.text[:30]}")
    ...
    def test_posts_view_follow_index(self) -> None:
        ...
        author = PostsViewTests.author
        Follow.objects.create(user=self.user, following=author)
